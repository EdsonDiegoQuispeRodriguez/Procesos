 ...
 * @author edson
 */
@WebServlet(name = "FinalizarInProcessServlet", urlPatterns = {"/FinalizarInProcessServlet"})
public class FinalizarInProcessServlet extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try ( PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet FinalizarInProcessServlet</title>");            
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet FinalizarInProcessServlet at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
            throws ServletException, IOException {
        String[] checkboxes = request.getParameterValues("checkboxName2");
        int numero = 0;
        if (checkboxes != null) {
            for (String checkbox : checkboxes) {
                numero = Integer.parseInt(checkbox);
                dao.editarOrdenServicioFinalizar(numero);
